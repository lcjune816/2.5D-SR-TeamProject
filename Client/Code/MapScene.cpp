...
HRESULT	MapScene::Ready_Scene() {
	...
	//pMiniGameLoading = CLoading::Create(GRPDEV, CLoading::LOADING_MINIGAME);
	//if (pMiniGameLoading == nullptr)
	//	return E_FAIL;
...
INT	 MapScene::Update_Scene(CONST FLOAT& _DT) {
	...
	if (TileManager::GetInstance()->Get_MiniGame() )
	{
		...
		{
				MiniGameScene* pMini = MiniGameScene::Create(GRPDEV, this);
				pMini->Start_MiniGame();
				TileManager::GetInstance()->Set_Stage();
				...
				SceneManager::GetInstance()->Set_CurrentScene(pMini);
				return 1;
		}
	} 
	else if (!TileManager::GetInstance()->Get_Loading())
	{
