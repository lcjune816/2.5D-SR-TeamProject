...
#define PLAY_BOSS_EFFECT(SKILL_TYPE, POSITION, SCALE, PLAYTIME)		EffectManager::GetInstance()->Append_Effect(EFFECT_OWNER::BOSS,					\
	BossEffect::Create(GRPDEV, SKILL_TYPE, POSITION, TRUE, PLAYTIME, SCALE));								// 보스 몬스터 : 이펙트 반복 실행
#define PLAY_BOSS_EFFECT_ONCE(SKILL_TYPE, POSITION, SCALE ,PLAYTIME)	EffectManager::GetInstance()->Append_Effect(EFFECT_OWNER::BOSS,				\
	BossEffect::Create(GRPDEV, SKILL_TYPE, POSITION, FALSE, PLAYTIME, SCALE));								// 보스 몬스터 : 이펙트 최초 한번 실행
