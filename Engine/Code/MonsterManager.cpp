...
FILENAMEINFO MonsterManager::Make_ID_from_Filename(const wstring& _Filename)
{
	...
	if (!wcscmp(tInfo.szType, L"Monster")) {
		tInfo.Type = (uint8_t)MONSTER_SEP::Monster;
		if (!wcscmp(tInfo.szName, L"BlueEvilBat")) {
			tInfo.name = (uint8_t)MONSTER_TYPE::Bat;
			if (!wcscmp(tInfo.szState, L"Stand"))						tInfo.State = (uint8_t)MONSTER_ANIM::Stand;
		}
		else if (!wcscmp(tInfo.szName, L"BlueScorpionEvilSoul")) {
			tInfo.name = (uint8_t)MONSTER_TYPE::ScorpionEvilSoul;
			if (!wcscmp(tInfo.szState, L"Stand"))					tInfo.State = (uint8_t)MONSTER_ANIM::Stand;
		...
		else if (!wcscmp(tInfo.szName, L"BlueEvilSlime")) {
			tInfo.name = (uint8_t)MONSTER_TYPE::EvilSlime;
			if (!wcscmp(tInfo.szState, L"Stand"))					tInfo.State = (uint8_t)MONSTER_ANIM::Stand;
		}
		else if (!wcscmp(tInfo.szName, L"BlueEliteShotGunEvilSoul")) {
			tInfo.name = (uint8_t)MONSTER_TYPE::ShotGunEvilSoul;
			if (!wcscmp(tInfo.szState, L"Stand"))						tInfo.State = (uint8_t)MONSTER_ANIM::Stand;
		}
	}
	else if (!wcscmp(tInfo.szType, L"Bullet")) {
		tInfo.Type = (uint8_t)MONSTER_SEP::Bullet;
		if (!wcscmp(tInfo.szName, L"Standard")) {
			tInfo.name = (uint8_t)BULLET_TYPE::Standard;
		...
		else if (!wcscmp(tInfo.szName, L"ScorpionBullet")) {
			tInfo.name = (uint8_t)BULLET_TYPE::ScorpionBullet;
			if (!wcscmp(tInfo.szState, L"Death"))					tInfo.State = (uint8_t)MONSTER_ANIM::Death;
		}
		else if (!wcscmp(tInfo.szName, L"Chain")) {
			tInfo.name = (uint8_t)BULLET_TYPE::Chain;
