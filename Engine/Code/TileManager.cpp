...
void TileManager::Save_Tile(HWND g_hWnd)
{
	...
	_vec3			 Rotation   = {};
		...
		for (auto& pTile : m_vecTileBuffer[MODE_TILE])
		{
			...
			WriteFile(hFile,  &cTileName,  sizeof(_tchar)*256,		&dwByte, NULL);
...
void TileManager::Load_Tile(HWND g_hWnd)
{
	...
	while (true)
	{
		...
		ReadFile(hFile, &cTileName, sizeof(_tchar*), &dwByte, NULL);
		...
		//dynamic_cast<TileInfo*>(obj->Get_Component(COMPONENT_TYPE::COMPONENT_TILEINFO))->Set_TileAll(cTileName, eTileSide, eTileState, eTileMode, iTilenum);
		//dynamic_cast<Transform*>(obj->Get_Component(COMPONENT_TYPE::COMPONENT_TRANSFORM))->Set_Scale(*Scale);
		//dynamic_cast<Transform*>(obj->Get_Component(COMPONENT_TYPE::COMPONENT_TRANSFORM))->Set_Rotation(*Rotation);
		//dynamic_cast<Transform*>(obj->Get_Component(COMPONENT_TYPE::COMPONENT_TRANSFORM))->Set_Pos(Info);
